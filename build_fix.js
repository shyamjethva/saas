...
const portfoliosToBuild = [
  { src: 'Portfolio/all portfolios/shyam portfolio/portfoliolatest', dest: 'shyam-portfolio' },
  { src: 'Portfolio/all portfolios/hiral_portfolio/hiral_portfolio', dest: 'hiral_portfolio' },
...
for (const p of portfoliosToBuild) {
  const fullSrc = path.join(basePath, p.src);
  const fullDest = path.join(basePath, 'frontend/public/portfolios', p.dest);

  if (!fs.existsSync(fullSrc)) {
    console.log(`Skipping ${p.dest}, src not found: ${fullSrc}`);
    continue;
  }
